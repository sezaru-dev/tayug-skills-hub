...
export async function PATCH (req: Request) {
  ...
  try {
    ...
    const parsed = HeaderFormSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const { fullname, headline, barangay } = parsed.data;

    // Proceed with updating profile header
    const updatedProfile = await ProfileRepository.upsertProfile({userId, fullname, headline, barangay: barangay ?? undefined })
